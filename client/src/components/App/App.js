...
const App = () => {
  ...
  useEffect(() => {
    const loginSuccessSubscription = userStore.current.emitter.addListener(
      "LOGIN_SUCCESS",
      () => {
        localStorage.setItem(
          "userData",
          JSON.stringify(userStore.current.data)
        );
        setIsAuthenticated(true);
      }
    );

    const logoutSuccessSubscription = userStore.current.emitter.addListener(
      "LOGOUT_SUCCESS",
      () => {
